...
export const usePagesNavigation = () => {
    ...
    const { setNavigationHeaderProps, navigationHeader } = useContext(NavigationHeaderDataContext)
    ...
    const handleGoToRegisterPage = useCallback(() => {
        ...
        setNavigationHeaderProps({ title: "", hasTitle: false, shouldRenderHeader: true, navigationHistory: newHistory })
        navigate(PATH.REGISTER)
    }, [navigate, setNavigationHeaderProps, navigationHeader])

    const handleGoBack = useCallback(() => {
        const previousPage = navigationHistory?.length ? navigationHistory[navigationHistory.length - 1] : undefined
        if (previousPage) {
            setNavigationHeaderProps(previousPage)
            navigate(-1)
        } else handleGoToLoginPage()
    }, [navigate, setNavigationHeaderProps, navigationHistory, handleGoToLoginPage])
